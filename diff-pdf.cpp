...
// Globals for color customization
//static int g_old_color[3] = {0, 0, 255}; // Default blue
//static int g_new_color[3] = {255, 0, 0}; // Default red
//static int g_unchanged_color[3] = {170, 230, 130}; // Default Green

static int g_old_color[3] = {0, 0, 255}; // Default blue
static int g_new_color[3] = {0, 255, 255}; // Default red
static int g_unchanged_color[3] = {230, 230, 230}; // Default Green

// Helper to parse r,g,b
bool parse_rgb(const wxString &value, int color[3])
{
    ...
    {
        long val;
        if (!parts[i].ToLong(&val) || val < 0 || val > 255)
            return false;
        color[i] = static_cast<int>(val);
    }
    return true;
}
             ...
             y++, data2 += stride2, out += stridediff)
        {
            ...
            for (int x = 0; x < r2.width * 4; x += 4)
            {
                ...
                else
                {
                    ...
                    *(out + x + 2) = cb2;
                    //*(out + x + 0) = g_new_color[0]; // R
                    //*(out + x + 1) = g_new_color[1]; // G

                    // ADSE: Override 
                    //*(out + x + 2) = g_new_color[2]; // B
                }
            }
            ...
            if (g_mark_differences && linediff)
            {
                for (int x = 0; x < (10 < r2.width ? 10 : r2.width) * 4; x += 4)
                {
                    ...
                    //*(out + x + 1) = 0;
                    //*(out + x + 2) = 255;
                    //*(out + x + 0) = g_old_color[0];
                    //*(out + x + 1) = g_old_color[1];
                    //*(out + x + 2) = g_old_color[2];
                }
            }
        }
    }

    // ADSE: Convert s1 region in datadiff to grayscale
    if (s1)
    {
        const unsigned char *data1_gray = cairo_image_surface_get_data(s1);
        unsigned char *out_gray = datadiff + r1.y * stridediff + r1.x * 4;

        for (int y = 0; y < r1.height; ++y, data1_gray += stride1, out_gray += stridediff)
        {
            for (int x = 0; x < r1.width * 4; x += 4)
            {
                // Sammenlign med original fra s1
                unsigned char r = data1_gray[x + 2];
                unsigned char g = data1_gray[x + 1];
                unsigned char b = data1_gray[x + 0];

                unsigned char r_out = out_gray[x + 2];
                unsigned char g_out = out_gray[x + 1];
                unsigned char b_out = out_gray[x + 0];

                // Hvis piksel ikke ble endret → gjør den hvit
                if (r == r_out && g == g_out && b == b_out)
                {
                    if (r_out >= 250 and g_out >= 250 and b_out >= 250) {
                        //
                    }
                    else {
                        unsigned char gray = to_grayscale(r, g, b);
                        out_gray[x + 0] = g_unchanged_color[0]; // B
                        out_gray[x + 1] = g_unchanged_color[0];    // G
                        out_gray[x + 2] = g_unchanged_color[0];       // R
                    }
    ...
    if (thumbnail)
    {
        ...
        if (!changes)
        {
            ...
            for (int i = thumbnail_width * thumbnail_height;
                 ...
                 i--, out += 3)
            {
                out[0] = g_unchanged_color[0] / 2 + out[0] / 2;
                out[1] = g_unchanged_color[1] / 2 + out[1] / 2;
                out[2] = g_unchanged_color[2] / 2 + out[2] / 2;
            }
        }
    }
...
// ADSE: THIS FUNCTION STARTS THE SHOW
...
int main(int argc, char *argv[])
{
    ...
    static const wxCmdLineEntryDesc cmd_line_desc[] =
        {
            ...
            // Override color for differences found on old file
            {wxCMD_LINE_OPTION,
             NULL, "old-color", "specify RGB comma-separated old image color (r,g,b)", wxCMD_LINE_VAL_STRING},

            // Override color for differences found on new file
            {wxCMD_LINE_OPTION,
             NULL, "new-color", "specify RGB comma-separated new image color (r,g,b)", wxCMD_LINE_VAL_STRING},

            // Override color for everything else where no difference
            {wxCMD_LINE_OPTION,
             NULL, "unchanged-color", "specify RGB comma-separated new image color (r,g,b)", wxCMD_LINE_VAL_STRING},
    ...
    wxString oldColorOpt, newColorOpt;
    if (parser.Found("old-color", &oldColorOpt))
    {
        if (!parse_rgb(oldColorOpt, g_old_color))
        {
            fprintf(stderr, "Invalid --old-color format. Use r,g,b\n");
            return 2;
        }
    }

    if (parser.Found("new-color", &newColorOpt))
    {
        if (!parse_rgb(newColorOpt, g_new_color))
        {
            fprintf(stderr, "Invalid --new-color format. Use r,g,b\n");
            return 2;
        }
    }

    if (parser.Found("unchanged-color", &newColorOpt))
    {
        if (!parse_rgb(newColorOpt, g_unchanged_color))
        {
            fprintf(stderr, "Invalid --new-color format. Use r,g,b\n");
            return 2;
        }
